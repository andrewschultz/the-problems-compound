Version 5 of Quip-Based Conversation Modified by Michael Martin begins here.

"An extension to Reactable Quips to allow for more traditional menu-based conversation."

Include Version 10 of Reactable Quips Modified by Michael Martin.
...
definition: a quip (called qu) is permissible:
	unless qu is a response listed in qbc_litany:
		decide no;
	choose row with response of qu in qbc_litany;
	if enabled entry > 0 and permit entry is 1:
		decide yes;
	decide no;
...
Understand "talk to [someone]" as talking to. [this makes it so we ask 'whom do you want to talk to']

Understand "talk to [something]" as talking to.

Understand "talk with [someone]" as talking to. [this makes it so we ask 'whom do you want to talk to']

Understand "talk with [something]" as talking to.

Understand "talk [someone]" as talking to. [this makes it so we ask 'whom do you want to talk to']

Understand "talk [something]" as talking to.

Understand "t [someone]" as talking to. [this makes it so we ask 'whom do you want to talk to']
		...
		if the enabled entry > 0:
			...
			if qbc_index is the number understood:
				if response entry is not permissible:
					say "[smack-back of response entry]";
					the rule fails;
				else:
...
to say smack-back of (my-resp - a quip):
	repeat through table of smackdowns:
		if smak-quip entry is my-resp:
			say "[smak-txt entry][one of][line break]Dang it. It's not the first time you thought of a clever response but balked at the last second.[or][line break][stopping]";
			display the QBC options;
			the rule succeeds;
	say "You'd like to say that, and someone with more courage can, but you can't right now.";
	display the QBC options; [ [i][bracket]NOTE: you can RECAP to see the other options.[close bracket][r][paragraph break]"]

Section 5 - Recaps

QBC recap is an action out of world applying to nothing.  Understand "repeat" or "recap" as QBC recap.

Carry out QBC recap (this is the perform QBC recap rule):
	let qbc_index be 0;
	unless QBC is active:
		say "You're not in a conversation, so you don't need to RECAP." instead;
	repeat through qbc_litany:
		if the enabled entry > 0:
			increase qbc_index by 1;
...
Quip-Based Conversation Modified ends here.
